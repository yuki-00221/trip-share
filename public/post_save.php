<?php
    ...
    if($post_id) {
        if($image_path) {
            $stmt = $pdo->prepare("
                UPDATE posts 
                SET title = :title, body = :body, prefecture = :prefecture, travel_date = :travel_date, image_path = :image_path 
            ...
            $stmt = $pdo->prepare("
                UPDATE posts 
                SET title = :title, body = :body, prefecture = :prefecture, travel_date = :travel_date 
                WHERE id = :id AND user_id = :user_id
            ");
            $stmt->bindValue(':title', $title, PDO::PARAM_STR);
            $stmt->bindValue(':body', $body, PDO::PARAM_STR);
        ...
        $stmt = $pdo->prepare("
            INSERT INTO posts (user_id, title, body, prefecture, travel_date, image_path, created_at) 
